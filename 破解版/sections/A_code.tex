...
本附录给出与正文结果一致、可复现关键指标的核心程序节选。运行环境：Python 3.11，依赖 \texttt{numpy}、\texttt{pandas}、\texttt{scipy}（\texttt{milp}/\texttt{brentq}）、\texttt{rasterio}（读取 $30\,$m DEM）；每问一个独立脚本（\texttt{q1\_1.py}、\texttt{q1\_21.py}、\texttt{q1\_22.py}、\texttt{q2\_1.py}、\texttt{q2\_2.py}、\texttt{q3\_1.py}、\texttt{q3\_2.py}、\texttt{q4\_1.py}、\texttt{q4\_2.py}），输入为附件 xlsx 与 DEM，输出为各问的 CSV 结果与汇总 txt。完整代码随附件提交。

\subsection{问题一：三段式能耗与最大安全载荷求解}
